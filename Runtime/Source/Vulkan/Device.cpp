...
QueueFamilyIndices Device::FindQueueFamilyIndices() const {
    ...
    for (int i = 0; i < queue_families.size(); i++) {
        ...
        vkGetPhysicalDeviceSurfaceSupportKHR(m_physical_device, i, m_surface.GetHandle(), &is_present_support);
...
std::vector<VkDeviceQueueCreateInfo> Device::BuildQueueCreateInfos(const QueueFamilyIndices& queue_indices) {
    ...
    const std::set queue_families = { queue_indices.graphics.value(),
                                      queue_indices.present.value(),
                                      queue_indices.compute.value() };
...
void Device::CreateDevice(const std::vector<const char*>&             required_extensions,
